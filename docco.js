...
(function() {
  ...
  parse = function(source, code, config) {
    ...
    var block_comment_start, matches, block_comment_end, block_comment_one_liner, block_comment_start_with, block_comment_end_with, block_comment_mid, in_comment_block;
    if(config.multi){
      block_comment_start = RegExp("^\\s*" + config.multi.start + "\\s*$");
      block_comment_end = RegExp("^\\s*" + config.multi.end + "\\s*$");
      block_comment_one_liner = RegExp("^\\s*" + config.multi.start + "\\s*(.*?)\\s*" + config.multi.end + "\\s*$");
      block_comment_start_with = RegExp("^\\s*" + config.multi.start + "\\s*(.*?)$");
      block_comment_end_with = RegExp("\\s*(.*?)\\s*" + config.multi.end + "\\s*$");
      block_comment_mid = RegExp("^\\s*" + config.multi.middle + "\\s?");
    }
    lines = code.split('\n');
    sections = [];
    ...
    for (_j = 0, _len1 = lines.length; _j < _len1; _j++) {
      ...
      if(in_comment_block){
        if(block_comment_end && line.match(block_comment_end)){
          ...
          line = '';
        }else if(block_comment_end_with && ( matches = line.match(block_comment_end_with))){
          in_comment_block = false;
          docsText += matches[1].replace(block_comment_mid || '', '') + '\n';
        }
        docsText += line.replace(block_comment_mid || '', '') + '\n';
      }else if(block_comment_one_liner && (matches = line.match(block_comment_one_liner))){
        if (hasCode) {
          save();
        }
        docsText += matches[1] + '\n';
      }else if(block_comment_start && line.match(block_comment_start)){
        in_comment_block = true;
        if (hasCode) {
          save();
        }
      }else if(block_comment_start_with && (matches = line.match(block_comment_start_with))){
        in_comment_block = true
        if (hasCode) {
          save();
        }
  ...
  write = function(source, sections, config) {
    ...
    if (firstSection) {
      ...
      demo = firstSection.docsText.match(/demo:\s*([^\s]*)/);
      if(demo){
        demo = demo[1];
      }
    }
  ...
  configure = function(options) {
    ...
    }, defaults, _.pick.apply(_, [options].concat(__slice.call(_.keys(defaults)))));
  ...
  languages =  require('docco/resources/languages.json');;
